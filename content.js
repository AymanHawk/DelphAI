$(document).ready(() => {
    ...
    function addButtons() {
        ...
        if (targetDiv.length) {
            ...
            const generateButton = $('<button>')
                ...
                .addClass('bg-blue-600 text-white px-2 py-1 m-0 mr-2 rounded inline-flex items-center')
                .click(() => {
                    simulateClick().then(() => captureEmailContents().then(contents => callOllamaApi(contents)));
            ...
            const improveButton = $('<button>')
                ...
                .addClass('bg-green-600 text-white px-2 py-1 m-0 mr-2 rounded inline-flex items-center')
                .click(() => {
                    simulateClick().then(() => captureEmailContents().then(contents => callImproveApi(contents)));
                }); // Add click event to capture emails and call API
            ...
            wrapperDiv.append(generateButton, improveButton, captureButton);
    ...
    function captureEmailContents() {
        return new Promise((resolve) => {
            setTimeout(() => { // Initial delay to ensure all elements are fully loaded
                ...
                emailContainers.each((index, element) => {
                    const emailContent = $(element).find('div[aria-label="Message body"]').text();
    ...
    const JSONformat = {
        ...
        suggestedResponse: "Hey Jared, I hope you're doing well. I wanted to follow up on the email I sent you last week. Please let me know if you have any updates. Best Regards, Ayman Haque",
    }
        ...
        examples: [
            {
                ...
                suggestedResponse: "Hi Jane, Thank you for your email. I appreciate the update and will follow up with the team. Best Regards, John Smith"
            },
            {
                tone: "Informal",
                recipientName: "Tom",
                recipientEmail: "tom@example.com",
                suggestedResponse: "Hey Tom, Got your email. Thanks for the heads-up. I'll get back to you soon. Cheers, Alex"
            },
            {
                tone: "Professional",
                recipientName: "Emily",
                recipientEmail: "emily@example.com",
                suggestedResponse: "Dear Emily, I hope this message finds you well. Thank you for your inquiry. I will look into the matter and revert shortly. Sincerely, Michael"
            }
        ]
    }
        ...
        fetch('http://localhost:11434/api/chat', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `You respond to emails using the email thread as context. The email thread will be provided to you and you will respond as if you are the email recipient. You only return responses as JSON. The response you will return will be in this format: ${JSONformat}. These are some example responses: ${sampleResponses || 'Not Provided Yet'} And your response will be in this tone ${JSONformat.tone}`
                    },
                    {
                        role: 'user',
        ...
        fetch('http://localhost:11434/api/chat', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are an assistant that improves the quality of email responses. I will pass the entire thread\'s separate emails as context and you will use that to provide an improved version of the response. You only respond with the improved response you generate, nothing else.'
                    },
                    {
                        role: 'user',
                        content: contents // Pass the captured email contents here
